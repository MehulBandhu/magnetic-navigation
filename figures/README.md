...
| `extension_real_data.png` | 2F | the models compared on the same real tiles: ratio to the Gaussian estimator (3.9 for the submitted model, 1.4 for the generator-v2 model at 72k steps and for the fine-tuned ones), per-tile errors before and after, and the kurtosis distribution of real against generated tiles |
| `scale_transfer.png` | 2G | the Gawler survey grid across pixel size at fixed h/dx and across altitude at fixed pixels: fitted slope, the Gaussian estimator against its own floor, the network against the estimator |
